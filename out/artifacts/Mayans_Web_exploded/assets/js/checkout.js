...
async function loadCheckoutData() {
    ...
    if (response.ok) { //200
        ...
        if (json.status) {
            ...
            // cart-details
            let st_tbody = document.getElementById("ms-tbody");
            let st_item_tr = document.getElementById("ms-item-tr");
            let st_subtotal_tr = document.getElementById("ms-subtotal-tr");
            let st_order_shipping_tr = document.getElementById("ms-order-shipping-tr");
            let st_order_total_tr = document.getElementById("ms-order-total-tr");
            ...
            cartItems.forEach(cart => {
                ...
                st_item_tr_clone.querySelector("#ms-product-title")
                    .innerHTML = cart.product.title;
                st_item_tr_clone.querySelector("#ms-product-qty")
                    .innerHTML = cart.qty;
                item_count += cart.qty;
                let item_sub_total = Number(cart.qty) * Number(cart.product.price);

                st_item_tr_clone.querySelector("#ms-product-price")
            ...
            st_subtotal_tr.querySelector("#ms-product-total-amount")
                .innerHTML = new Intl.NumberFormat(
                    "en-US",
                    { minimumFractionDigits: 2 })
                    .format(total);
            ...
            city_select.addEventListener("change", (e) => {
                ...
                st_order_shipping_tr.querySelector("#ms-product-shipping-charges")
                    .innerHTML = new Intl.NumberFormat(
                        "en-US",
                        { minimumFractionDigits: 2 })
                ...
                st_order_total_tr.querySelector("#ms-order-total-amount")
                    .innerHTML = new Intl.NumberFormat(
                        "en-US",
                        { minimumFractionDigits: 2 })
