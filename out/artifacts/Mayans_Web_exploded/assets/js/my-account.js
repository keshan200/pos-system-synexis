...
async function getUserData() {
    ...
    if (response.ok) {
        ...
        document.getElementById("welcomeusername").innerHTML = `Hello, ${json.firstName}`;
        document.getElementById("username").innerHTML = `Hello, ${json.firstName} ${json.lastName}`;
        document.getElementById("since").innerHTML = `Mayans Member Since ${json.since}`;
...
async function saveChanges() {
    ...
    if (response.ok) {
        ...
        if (json.status) {
            ...
            await getUserData(); // reload user data from server
            setTimeout(() => {
        location.reload();
    }, 1000);
        } else {
