...
async function searchProduct(firstResult) {
    ...
    const sort_value = document.getElementById("ms-sort").value;
...
const st_product = document.getElementById("ms-product"); // product card parent node
let st_pagination_button = document.getElementById("ms-pagination-button");
let current_page = 0;

function updateProductView(json) {
    const product_container = document.getElementById("ms-product-container");
    product_container.innerHTML = "";
    json.productList.forEach(product => {
        let st_product_clone = st_product.cloneNode(true);// enable child nodes cloning / allow child nodes
        st_product_clone.querySelector("#ms-product-a-1").href = "single-product.html?id=" + product.id;
        st_product_clone.querySelector("#ms-product-img-1").src = "product-images//" + product.id + "//image1.png";
        st_product_clone.querySelector("#ms-product-add-to-cart").addEventListener(
            "click", (e) => {
                addToCart(product.id, 1);
                e.preventDefault();
            });
        st_product_clone.querySelector("#ms-product-a-2").href = "single-product.html?id=" + product.id;
        st_product_clone.querySelector("#ms-product-title-1").innerHTML = product.title;
        st_product_clone.querySelector("#ms-product-price-1").innerHTML = new Intl.NumberFormat(
            "en-US",
    ...
    let st_pagination_container = document.getElementById("ms-pagination-container");
